...
???
Il existe plusieurs manière de cartographier et interpréter des données ponctuelles.
On peut citer 3 manière usuelles de procéder.

1) On peut agréger ces données dans des polygons s'apppuyant sur un découpage, généralement administratif du territoire (les départements, les communes, etc.).

Ces découpage de l'espace peuvent générer un effet MAUP (modifiable areal unit problem) et biaiser notre représentation des phénomène.
...
Des grilles carroyées composées de petits carreaux peuvent permettre, par exemple, d'observer des phénomènes infracommunaux. Néanmoins, le rendu peut avoir un aspect très erratique et limiter l'interprétabilité de nos cartes.


3) Enfin, le lissage spatial est une méthode clé pour analyser la répartition spatiale des données disponibles à un niveau géographique fin, en fournissant une cartographie claire et simple, débarrassée de l'arbitraire délimitations administratives.
...
Notamment spatstats, très complet, qui permet notamment d'uiliser des algorithme de choix du rayon de lissage optimal.
Mais utilise les Transformations de fourier rapide, qui posent problème dans la gestion des effets de bord, particulièrement important en analyse urbaine.
...
**Les hypothèses de lissage**

  + Un **noyau** (*kernel*) indiquant la manière d'approcher le voisinage ;
  
  + Le **rayon de lissage** quantifiant la taille du voisinage (arbitrage biais/variance) ;
...
**Dans btb...**
  ...
  + _Pris en compte. **Méthode conservative**_
]
